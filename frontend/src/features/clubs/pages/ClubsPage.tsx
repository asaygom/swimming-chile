  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-ink tracking-tight">Clubes</h1>
          <p className="text-content-subtle mt-1">Explora los clubes de natación registrados.</p>
        </div>
        <div className="flex w-full flex-col gap-3 md:w-auto sm:flex-row">
          ...
          <select
            ...
            className="w-full rounded-lg border border-line bg-surface px-4 py-2 text-sm text-content-muted shadow-sm outline-none transition-shadow focus:border-brand-pool focus:ring-2 focus:ring-brand-pool sm:w-52"
          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {data.data.map((club) => (
                  <Link 
                    key={club.id} 
                    to={`/clubs/${club.id}`}
                    className="group bg-surface rounded-xl shadow-sm border border-line p-6 hover:shadow-md hover:border-brand-pool transition-all flex flex-col h-full"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-canvas text-action flex items-center justify-center font-bold text-xl group-hover:bg-brand-cyan group-hover:text-ink transition-colors">
                        {club.name.charAt(0)}
                      </div>
                      <div>
                        <h2 className="text-lg font-bold text-ink leading-tight group-hover:text-action transition-colors">{club.name}</h2>
                        <p className="text-sm text-content-subtle mt-0.5 flex items-center gap-1">
                    ...
                    <div className="mt-6 pt-4 border-t border-line flex justify-between items-center">
                      <span className="text-sm font-medium text-content-muted">Nadadores registrados:</span>
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-canvas text-ink">
