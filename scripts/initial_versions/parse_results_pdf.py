...
try:
    ...
    raise SystemExit("[ERROR] Falta pdfplumber. Instálalo con: pip install pdfplumber") from exc
...
def parse_pdf(pdf_path: Path) -> Tuple[List[ParsedResultRow], pd.DataFrame, ParseStats, Dict[str, Optional[str]]]:
    ...
    return rows, debug_df, stats, metadata



def build_output_frames(parsed_rows: List[ParsedResultRow], competition_id: Optional[int], default_source_id: Optional[int]) -> Dict[str, pd.DataFrame]:
    ...
    for row in parsed_rows:
        ...
        if row.event_name not in seen_events:
            seen_events.add(row.event_name)
            event_match = EVENT_HEADER_RE.match(f"Event {row.event_number} {row.event_name}")
            if not event_match:
                fail(f"No se pudo reconstruir metadata del evento: {row.event_name}")
            event_records.append(
                {
                    "competition_id": str(competition_id) if competition_id is not None else None,
                    "event_name": row.event_name,
                    "stroke": event_match.group("stroke").strip().lower(),
                    "distance_m": event_match.group("distance_m").strip(),
                    "gender": event_match.group("gender").strip().lower(),
            ...
            athlete_records.append(
                {
                    ...
                    "gender": event_match.group("gender").strip().lower() if event_match else None,
...
def save_outputs(frames: Dict[str, pd.DataFrame], debug_df: pd.DataFrame, metadata: Dict[str, Optional[str]], out_dir: Path, excel_name: str) -> None:
    ...
    with pd.ExcelWriter(workbook_path, engine="openpyxl") as writer:
        for sheet_name in ["club", "event", "athlete", "result", "raw_result"]:
...
def main() -> None:
    ...
    parsed_rows, debug_df, stats, metadata = parse_pdf(pdf_path)

    if not parsed_rows:
        fail("No se extrajeron filas de resultados. Revisa el layout del PDF o el archivo de debug.")

    frames = build_output_frames(parsed_rows, args.competition_id, args.default_source_id)
    ...
    print(f"Filas de resultado:       {stats.result_rows_found}")
