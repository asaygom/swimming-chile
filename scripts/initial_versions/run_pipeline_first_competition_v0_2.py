#!/usr/bin/env python3
"""
run_pipeline_first_competition_v0_2.py

Mejoras v0.2:
- Normaliza result_time_text
- Evita duplicados básicos con NOT EXISTS
- Entrega mejor resumen final
- Sigue usando competition_id y default_source_id como parámetros
- Asume que source, pool y competition ya existen en core
"""
...
def normalize_swim_time_text(value):
    ...
    if m:
        ...
        if total_minutes > 0:
            return f"{total_minutes}:{seconds:02d}.{centis:02d}"
        return f"{seconds}.{centis:02d}"

    m = re.fullmatch(r"(\d{1,3}):(\d{2})(?:\.(\d{1,6}))?", value)
    if m:
    ...
    m = re.fullmatch(r"(\d{1,3})(?:\.(\d{1,6}))?", value)
    if m:
